...
//
// 2023/04/13 ver1.0β插件發布
// 2023/05/01 ver1.09 規格修正
// - 訊息視窗的顯示時間已加快。
// - 修正了從已儲存的資料重新啟動時，memory_talk 中的第一個對話不會回應的問題。
// 2023/05/05 ver1.10 規格修正
// - 修正了當超出 Memory_talk 設定數量時系統角色被刪除的錯誤。
// 2023/05/06 ver1.11 規格修正
// - 修正了不使用 memory_talk 的答案未儲存在變數中的錯誤。
// - 調整等待回應時的處理。
// 2023/05/09 新增 ver1.2 規範
// - 現在可以在插件參數中設定自訂字體。
// - 現在可以在插件命令中設定角色名稱、臉部圖形和索引。
// 2023/05/09 ver1.21 緊急修復
// - 修復了等待模式控制。
// 2023/05/10 ver1.22 規格修正
// - 再次調整等待模式控制。
// - 當執行的事件具有固定方向時，現在它會在完成後回到原始方向。
// 2023/05/11 ver1.23 規格修正
// - 錯誤訊息輸出已翻譯成日文。
// 2023/05/14 ver1.24 規格修正
// - 修正了並行執行事件時發生錯誤的問題。
// 2023/05/15 新增 ver1.3 規範
// - 現在可以透過參數設定答案視窗的設計。
//
// --------------------------------------------------------------------------------------
/*:
 * @target MZ
 * @plugindesc 與 ChatGPT API 通訊並具有 AI 創建對話的插件
 * @author kotonoha*(中文化 安可)
